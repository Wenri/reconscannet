...
from if_net.models.training_UDA import TrainerUDA as Trainer
from scannet.UDADataset import UDADataset
...
def train_one_epoch(train_loader, abnormal_loader, trainer, epoch_it, print_every):
    ...
    total_steps = epoch_it * len(train_loader)
...
def main(args):
    ...
    abnormal_dataset = UDADataset('train', cfg)
    ...
    while trainer:
        epoch_it += 1
        train_one_epoch(train_loader, uda_loader, trainer, epoch_it, print_every)
...
def parse_args():
    ...
    parser.add_argument('--config', type=Path, default=base_dir / 'configs' / 'config_files' / 'if_net_uda.yaml',
