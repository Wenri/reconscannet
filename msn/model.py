...
from .MDS.MDS_module import minimum_density_sample, gather_operation
from .expansion_penalty.expansion_penalty_module import expansionPenaltyModule
...
class MSN(nn.Module):
    ...
    def forward(self, x):
        ...
        resampled_idx = minimum_density_sample(xx[:, 0:3, :].transpose(1, 2).contiguous(), out1.shape[1],
                                               mean_mst_dis)
        xx = gather_operation(xx, resampled_idx)
        delta = self.res(xx)
        xx = xx[:, 0:3, :]
