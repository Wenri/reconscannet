...
def run(opt, cfg):
    ...
    generator = Generator(network, 0.2, 'sample_input', resolution=opt.retrieval_res, batch_points=opt.batch_points)
    ...
    for cur_iter, data in enumerate(dataloader):
        ...
        for idx in c.box_label_mask.nonzero(as_tuple=True)[0]:
            ...
            z = network.get_z_from_prior((1,), sample=False, device=device)
            logits = generator.generate_mesh(z, features, voxels)
